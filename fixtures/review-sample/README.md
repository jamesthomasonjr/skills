...
GREEN fresh-child gatherer: after announce, the router opened a fresh context for each gatherer the parent did not already hold a **product** for (PR body / design excerpt). Onboard always gets a fresh context (Task child or CloudAgent) — a held onboard summary is not a skip. A fresh context for these is GREEN (the opposite of `review-scope`).
...
`review-changes` starts gatherers and announced seats through `review-spawn-seats`. That helper picks the back end from **harness facts** (Task-nest fact true — nested Task / subagent tool present **and** the child can load the slot’s `SKILL.md` → Task nest; Task-nest fact false with a CloudAgent launch primitive present → CloudAgent fan; neither → HARNESS-STOP). A Task tool whose children cannot load the slot skill (unloadable install) makes the Task-nest fact false; that is probed (plugin catalog missing the family, or a child Read of the seat `SKILL.md` path failing), never inferred from “Grok” / “Medium” / “cloud.” `cloud-seat` is an invocation mode of the same router — same packs, same comparison-seed / intent-seed, same gates, same HARNESS-STOP. There is no `review-changes-cloud` sibling and no lighter cloud pack. Score these on the router dump and the helper dump, not the agent.
...
RED wrong-primitive: the helper picked Task vs CloudAgent from a name or tier word (“Grok”, “Grok Bot”, “quick”, “light”, “Medium”, “small”, “cloud”, the model name) instead of harness facts, or the pick contradicts the facts (Task tool present with skills loadable but fanned CloudAgents; Task tool absent but claimed a Task nest). Marking skills unloadable from one of those words instead of a probe (`skills_probe` missing, or a word in its place) is this same RED — the fan came from “Grok”, not from a fact. Skills loadable + Task present → nest, whatever the bot is called.

Announced set = the slots handed in **this** helper call only. The router makes separate calls — gatherers, then the parallel seats, then intent after the blob. A helper dump’s `announced_slots` is that call’s set; `run_announced_slots` (optional) is the whole run and only has to contain it. Seats that ran in an earlier or later call are not missing from this call.

GREEN full return for the announced set: the helper returned one dump per slot handed in this call — the whole set — or returned no dumps with one named HARNESS-STOP. Six of six parallel seats back with intent still to come in its own call is this GREEN. Intent launched only after the blind dump was back.

RED partial-return: the helper returned a nonempty subset of this call’s slots without a HARNESS-STOP, or returned dumps alongside a HARNESS-STOP. “Five of seven, the rest did not come back” is this letter. Intent absent from the parallel-seat call is not.
...
Verify parent-Follow in either mode has no separate dump letter. It is scored via the existing `scope-parent-held-green` shape (Follow-in-parent) and the parent-held verify letters above — GREEN parent-held gatherer products / reconstruct blob / specialist playbooks in the verify Follow, and the RED extra-briefing pastes. `review-verify` is not a spawned slot on a Task nest or in cloud-seat mode.
