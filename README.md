...
- **[review-changes](./skills/engineering/review-changes/SKILL.md)** — Router for defect-first code review. Use when reviewing a change, PR, commit, or working tree. Uses `review-scope` (Follow-in-parent), then fans gatherers as fresh children, then a caller-named pack (`full` default / `core` opt-in), then the verifier on every announced seat list; does not review.
