...
- **[review-changes](./skills/engineering/review-changes/SKILL.md)** — Router for defect-first code review. Use when reviewing a change, PR, commit, or working tree. Uses `review-scope`, then fans out to seats then the verifier; does not review.
...
Live path: a wrapper may prime the parent with catch-me-up / orient-* on the file list [review-scope](./skills/engineering/review-scope/SKILL.md) returned, then invoke `review-changes`. The parent may hold those dumps. `review-changes` uses `review-scope` then fans out to [review-intent](./skills/engineering/review-intent/SKILL.md) + [review-blind](./skills/engineering/review-blind/SKILL.md) + [review-security](./skills/engineering/review-security/SKILL.md), then [review-verify](./skills/engineering/review-verify/SKILL.md) applies `gates.md`. Seats still run in fresh contexts. The security playbook is only in the security window.
