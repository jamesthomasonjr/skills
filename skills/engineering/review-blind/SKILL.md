...
**not** pass the PR body or commit message. Do not fetch them. Do not
read a procedure dump the router withheld.

Fetching the PR body or commit message is a Failure. Running in a
window that already had the PR body, onboard, or implementing turn is
also a Failure — even if this skill says not to use it.
...
- Start in a fresh context that contains only the router-passed comparison. A window that already had the PR body, onboard, or implementing turn is a Failure.
...
- Running in a window that already had the PR body, onboard, or implementing turn
