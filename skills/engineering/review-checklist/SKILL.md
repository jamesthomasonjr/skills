...
description: >-
  Checklist-candidate seat for defect-first review. Use when
...
Checklist-candidate seat. Generate **candidates** only. This skill does
**not** apply gates and does **not** write Findings / Assessment / Close.

This seat’s window is the **comparison plus its own playbook**, plus
a user-named checklist file **when they named one**. Start in a
fresh context. The router passes the comparison, comparison
command, file list, mixed-turn fix request if any, an optional
focus phrase, and the checklist path only if the caller named one.
It does **not** pass the playbook. It does **not** pass the onboard
product. It does **not** pass the reconstructed-intent blob. It
does **not** invent a checklist path. Onboard has no seat to seed.
This seat still fans with `review-blind`. The blob is not in this
window.

If no checklist path was passed: write exactly `No candidates.`
Do not invent a path. Do not Read a default checklist file.
...
- Start in a fresh context whose window is the comparison plus this seat’s own playbook, plus a user-named checklist file when they named one. Child Read of `playbook.md` is GREEN. Do not Read the reconstructed-intent blob. A window that already had the blob is a Failure.
- If the router passed no checklist path: write exactly `No candidates.` Do not invent a path.
...
2. If no user-named checklist path was passed: write exactly `No candidates.` Stop. Do not invent a path. Do not Read a default checklist file.
3. Read the passed checklist path. Inspect the complete comparison. For working tree: `git diff HEAD` **and** every untracked path in the file list. Untracked files are first-class — Read each, or `git diff --no-index -- /dev/null <path>`. Do not skip them because they are absent from `git diff HEAD`. Do not `git add`.
   For a named patch or branch/PR three-dot: inspect that complete diff, plus enough surrounding code to name a candidate.
4. Continue through the whole checklist and the whole diff after the first candidate. Each failed item is a candidate. Do not stop at one. Do not read files outside the file list except to demonstrate a call path for a candidate that already overlaps the diff.
5. Write the candidate list. Include leftovers the comparison still shows. Stop. Do not apply gates. Do not assign P0–P3. Do not write the envelope.
...
| “They forgot the path — I’ll use CHECKLIST.md” | Inventing a path is a Failure. Write exactly `No candidates.` |
| “Focus phrase is close enough to a checklist” | Focus is not a file. Seed no file unless they named one. |
...
| “Audit the whole repo; the checklist is the codebase” | Comparison only. This seat is not an existing-codebase audit. |
...
- Inventing a checklist path, or Reading a default file when none was passed
