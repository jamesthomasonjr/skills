...
| Chore / cleanup / tech debt; rename / migrate / wire CI / “add a retry\|index\|log\|guard”; known bug with clear locus **and no As-a framing**; spike with **one clear decision question** | `shape-task` |
| Vague / intermittent / no locus (“something’s broken”) | **Stop** — hand off to diagnosing/triage; do not shape |
| Foggy multi-question exploration (“figure out X,” “research the space,” bare “research …” with no sharp question) | **Stop** — hand off to wayfinder/research/grill; do not shape |
| Two adjacent levels both fit | ask once (see below) |

Prefer the **largest** fitting level. Bare “add X” is **not** enough for a feature — only when X is a user-perceivable product capability. Engineering “add …” chores and spikes with one clear decision question are tasks. **Known bugfixes:** As a / I want / so that (user-framed) → `shape-story`; otherwise a clear locus → `shape-task`. User-framed wins when both match. **Research:** a sharp single question → spike flavor; bare or multi-question “research …” → out-of-family handoff (foggy wins when the question is not sharp).
