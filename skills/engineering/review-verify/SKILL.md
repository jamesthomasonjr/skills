...
- Verify the merged seat candidates only. Do not generate a fresh candidate list from the diff. Do not tour the rest of the repo. Three candidate lists — not gatherer dumps as extra briefing.
...
1. Take all three seat candidate lists. Combine, dedupe, organize. Do not add a candidate that no seat emitted. Do not wait on two lists. Do not take gatherer dumps as extra briefing.
...
| “Paste gatherer dumps so verify has context” | Three lists. Gatherer extra-briefing is RED. |
