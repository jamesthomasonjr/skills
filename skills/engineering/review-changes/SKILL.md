...
- Start gatherers and announced seats through [../review-spawn-seats/SKILL.md](../review-spawn-seats/SKILL.md). That helper picks nested Task vs CloudAgent from **harness facts** (the tool list, a launch primitive) — never from “Grok,” “quick,” “light,” “Medium,” “small,” or “cloud.” It returns the **whole** announced set or one named HARNESS-STOP. A subset back from the helper is that HARNESS-STOP: stop, do not merge the seats that did return, do not re-announce `core`.
...
- **Task nest** — this host has a nested Task / subagent tool. Each gatherer and seat is a fresh child.
...
The helper returns every announced dump or one named HARNESS-STOP. Take the dumps as bytes. If it returns fewer dumps than announced slots, that is the HARNESS-STOP — write it and stop. Do not hand the subset to `review-verify`.
