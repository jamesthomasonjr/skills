...
- **[review-changes](./review-changes/SKILL.md)** — Router for defect-first code review. Use when reviewing a change, PR, commit, or working tree. Classifies the comparison; fans out to seats then the verifier; does not review.
...
Live path: a wrapper may prime the parent with catch-me-up / orient-* on the cheap-resolve file list, then invoke `review-changes`. The parent may hold those dumps. Seats still run in fresh contexts: [review-intent](./review-intent/SKILL.md) + [review-blind](./review-blind/SKILL.md) + [review-security](./review-security/SKILL.md), then [review-verify](./review-verify/SKILL.md) applies `gates.md`. The security playbook is only in the security window. Inspired by [AntJanus/skillbox](https://github.com/AntJanus/skillbox) (verifier impact floor; isolation is a fresh context, not instruction-withhold) and [yeameen/claude-code-review-council](https://github.com/yeameen/claude-code-review-council) (blind-context compose).
